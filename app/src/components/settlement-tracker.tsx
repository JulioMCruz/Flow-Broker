      ...
      <div className="flex-1 p-3 text-xs overflow-y-auto">
        ...
        {tab === "ens" && (
          <div className="space-y-3">
            ...
            <div className="mt-2">
              <a href="https://sepolia.app.ens.domains/perkmesh.eth" target="_blank" rel="noopener noreferrer"
                className="text-blue-600 hover:underline text-[10px]">View perkmesh.eth on ENS →</a>
            </div>
          </div>
        )}
        ...
        {tab === "cre" && (
          <div className="space-y-3">
            <p className="text-gray-400 uppercase tracking-wide text-[10px]">Chainlink CRE Workflows</p>
            {cre.map(w => (
              <div key={w.name} className="bg-gray-50 rounded p-2">
                ...
                <p className="text-gray-400 mt-0.5">{w.trigger}</p>
                <p className="text-gray-500 font-mono mt-0.5 text-[10px]">{w.lastResult}</p>
                <a href={`${EXPLORER}/${w.contract}`} target="_blank" rel="noopener noreferrer"
                  className="text-blue-600 hover:underline text-[10px]">contract →</a>
              </div>
            ))}
          </div>
        )}
