...
function calculatePrices(agents, ethPriceUsd) {
  const priceAdjustment = Math.max(-0.5, Math.min(0.5, (ethPriceUsd - 2000) / 20000));
  return agents.map((agent) => {
    const adjusted = Math.round(agent.basePrice * agent.costMultiplier * (1 + priceAdjustment));
...
function onPriceUpdate(runtime2) {
  ...
  const priceData = httpClient.sendRequest(runtime2, (sendRequester) => {
  ...
  runtime2.log(`ETH/USD price: $${priceData}`);
  const agentIds = config.agents.map((a) => BigInt(a.id));
  const newPrices = calculatePrices(config.agents, priceData);
  runtime2.log(`Calculated prices for ${agentIds.length} agents: [${newPrices.join(", ")}]`);
  const reportData = encodeAbiParameters(parseAbiParameters("uint256[] agentIds, uint256[] prices"), [agentIds, newPrices]);
...
var initWorkflow = (config) => {
  ...
  return [
    cre.handler(cron.trigger({ schedule: "0 */30 * * * *" }), onPriceUpdate)
  ];
};
async function main() {
