...
function onHealthCheck(runtime2) {
  const config = runtime2.config;
  const httpClient = new cre.capabilities.HTTPClient;
  const downAgents = [];
  const upAgents = [];
  for (const agent of config.agents) {
    const statusCode = httpClient.sendRequest(runtime2, (sendRequester) => {
    ...
    if (statusCode === 200) {
      ...
      runtime2.log(`Agent "${agent.name}" (id=${agent.id}) is DOWN (status: ${statusCode})`);
    }
  }
  runtime2.log(`Health check complete: ${upAgents.length} healthy, ${downAgents.length} down`);
  if (downAgents.length > 0) {
    runtime2.log(`Down agents: ${downAgents.join(", ")}`);
  }
  return downAgents.length === 0 ? "all-healthy" : `${downAgents.length}-agents-down`;
}
var initWorkflow = (config) => {
  const cron = new cre.capabilities.CronCapability;
  return [
    cre.handler(cron.trigger({ schedule: "0 */5 * * * *" }), onHealthCheck)
  ];
};
async function main() {
