...
init_toHex();
init_keccak256();
var PAYMENT_THRESHOLD_EVENT_ABI = parseAbi([
  "event PaymentThresholdReached(uint256 indexed batchId, uint256 count, uint256 totalAmount)"
]);
var GET_BATCH_STATS_ABI = parseAbi([
  "function getCurrentBatchStats() view returns (uint256 batchId, uint256 count, uint256 amount, uint256 threshold_)"
]);
var PAYMENT_THRESHOLD_EVENT_HASH = keccak256(toHex("PaymentThresholdReached(uint256,uint256,uint256)"));
function onPaymentThreshold(runtime2, log) {
  ...
  runtime2.log(`PaymentThresholdReached — batch=${batchId}, count=${count}, amount=${totalAmount}`);
  ...
  const readResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: config.evms[0].paymentAccumulatorAddress,
      data: callData
    })
  }).result();
